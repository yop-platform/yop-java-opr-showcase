$(function () {
    ...
    $('#select_user_address_province').change(function () {
        $.ajax({
            type: "GET",
            url: session.getItem("contextUrl")+"/address/" + $(this).val(),
            data: null,
            dataType: "json",
...
function uploadImage(fileDom) {
    ...
    //上传图片
    $.ajax({
        url: session.getItem("contextUrl")+"/user/uploadUserHeadImage",
        type: "post",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function (data) {
            if (data.success) {
                $(fileDom).prev("img").attr("src",session.getItem("contextUrl")+"/res/images/item/userProfilePicture/"+data.fileName);
