$(function () {
    ...
    $(".loginForm").unbind("submit").submit(function () {
        ...
        if (yn) {
            $.ajax({
                type: "POST",
                url: session.getItem("contextUrl")+"/login/doLogin",
...
function getGuessLoveProducts() {
    $.ajax({
        type: "GET",
        url: session.getItem("contextUrl")+"/guess/" + $("#tid").val(),
            ...
            if (data.success) {
                ...
                for (var i = 0; i < data.loveProductList.length; i++) {
                    ...
                    $(".context_ul_goodsList").children("ul").append("<li class='context_ul_main'><div class='context_ul_img'>" +
                        "<a href='/yop-demo/product/" + product_id + "'><img src='/yop-demo/res/images/item/productSinglePicture/" + src + "'/></a><p>¥" + product_sale_price + ".00</p></div></li>"
                    );
                }
            }
        }
