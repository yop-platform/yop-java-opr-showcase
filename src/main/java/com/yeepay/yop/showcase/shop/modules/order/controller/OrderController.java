package com.yeepay.yop.showcase.shop.modules.order.controller;
...
import com.yeepay.yop.showcase.shop.modules.order.service.PayOrderRequestService;
import com.yeepay.yop.showcase.shop.modules.order.vo.PayOrderRequestDTO;
import com.yeepay.yop.showcase.shop.modules.product.entity.*;
import com.yeepay.yop.showcase.shop.modules.product.service.*;
...
public class OrderController extends BaseController {
    ...
    public String goToPage(HttpSession session, Map<String, Object> map,
        ...
        List<ProductOrder> productOrderList = productOrderService.getList(
                new ProductOrder().setProductOrder_user(user)
        ...
        if (productOrderList.size() > 0) {
            ...
            logger.info("获取订单项信息及对应的产品信息{}", JSON.toJSONString(orderCount));
            for (ProductOrder order : productOrderList) {
                ...
                if (productOrderItemList != null) {
                    for (ProductOrderItem productOrderItem : productOrderItemList) {
                        ...
                        List<ProductImage> list = productImageService.getList(
                                product_id, (byte) 0, new PageUtil(0, 1)
                        );
                        logger.info("获取到的产品照片为{}", JSON.toJSONString(list));
                        product.setSingleProductImageList(list);
                        productOrderItem.setProductOrderItem_product(product);
                        if (order.getProductOrder_status() == 3) {
                            productOrderItem.setIsReview(true);
                        }
                    }
                }
    ...
    public String goToOrderPaySuccessPage(Map<String, Object> map, HttpSession session,
                                          @PathVariable("order_code") String order_code, HttpServletRequest request) {
        ...
        if (userId != null) {
            ...
            return "redirect:/login?url=" + requestURL;
        }
        logger.info("------验证订单信息------");
        logger.info("查询订单是否存在");
        ProductOrder order = productOrderService.getByCode(order_code);
    ...
    public String orderPay(HttpSession session, @PathVariable("order_code") String order_code) {
        ...
        //创建订单
        try {
            PayOrderRequestDTO requestDTO = buildYeepayRequestParam(order);
            String redirectUrl = payOrderRequestService.payOrderRequest(requestDTO);
            logger.info("获取收银台链接返回结果为{}", redirectUrl);
    ...
    private PayOrderRequestDTO buildYeepayRequestParam(ProductOrder order) {
        PayOrderRequestDTO requestDTO = new PayOrderRequestDTO();
        requestDTO.setIndustrySolutionId(1);
        getOrderInfo(order, requestDTO);
        requestDTO.setProductOrderId(order.getProductOrder_code());
        requestDTO.setCreateTime(new Date());
        return requestDTO;
    }

    private void getOrderInfo(ProductOrder order, PayOrderRequestDTO requestDTO) {
        ...
        for (ProductOrderItem productOrderItem : productOrderItemList) {
            ...
            Product product = productService.get(productOrderItem.getProductOrderItem_product().getProduct_id());
            goodsName = product.getProduct_name();
        }
        requestDTO.setAmount(orderAmount);
        requestDTO.setGoodsName(goodsName);
    }
    ...
    public String createOrderByOne(HttpSession session, Map<String, Object> map, HttpServletResponse response,
        ...
        logger.info("整合订单对象");
        User user = new User();
        user.setUser_id((Integer) userId);
        ProductOrder productOrder = new ProductOrder()
        ...
        productOrder = productOrderService.getByCode(productOrder_code.toString());
        logger.info("整合订单项对象");
        ProductOrderItem productOrderItem = new ProductOrderItem()
                ...
                .setProductOrderItem_order(new ProductOrder().setProductOrder_id(productOrder.getProductOrder_id()));
    ...
    public String createOrderByList(HttpSession session, Map<String, Object> map, HttpServletResponse response,
        ...
        for (ProductOrderItem orderItem : productOrderItemList) {
            orderItem.setProductOrderItem_order(new ProductOrder().setProductOrder_id(productOrder.getProductOrder_id()));
