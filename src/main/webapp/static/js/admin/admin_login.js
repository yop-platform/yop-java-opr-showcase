$(function () {
    ...
    $("#btn_login").click(function () {
        ...
        $.ajax({
            url: session.getItem("contextUrl")+"/admin/login/doLogin",
            type:"post",
            ...
            success:function (data) {
                ...
                if (data.success) {
                    ...
                    location.href = session.getItem("contextUrl")+"/admin";
                } else {
...
function initialCookie() {
    ...
    if(document.cookie.length>0) {
        ...
        if(url !== null) {
            ...
            $("#div_background").css("background-image", "url(/yop-demo/res/images/admin/loginPage/background-4.jpg)");
        }
        if(username !== null){
        ...
        $("#div_background").css("background-image", "url(/yop-demo/res/images/admin/loginPage/background-1.jpg)");
    }
}
//初始化页面数据
...
function getUserProfilePicture(username) {
    if(username !== null && username !== ""){
        $.getJSON(session.getItem("contextUrl")+"/admin/login/profile_picture",{"username":username},function (data) {
            if(data.success){
                if(data.srcString !== null){
                    $("#img_profile_picture").attr("src", session.getItem("contextUrl")+"/res/images/item/adminProfilePicture/" + data.srcString);
                    return true;
                }
            }
        });
    }
    $("#img_profile_picture").attr("src",session.getItem("contextUrl")+"/res/images/admin/loginPage/default_profile_picture-128x128.png");
}