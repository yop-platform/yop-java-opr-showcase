...
<head>
    ...
    <title>确认收货 - yeepay.com易宝-理想生活上易宝</title>
    <script>
        ...
        <table class="table_order_orderItem">
            ...
            <c:forEach items="${requestScope.productOrder.productOrderItemList}" var="orderItem" varStatus="i">
                <tr class="tr_product_info">
                    <td width="500px"><img
                            src="${pageContext.request.contextPath}/static/images/item/productSinglePicture/${orderItem.productOrderItem_product.singleProductImageList[0].productImage_src}"
...
<%--                    <li>请收到货后，再确认收货！否则您可能钱货两空！</li>--%>
                ...
                <script>
                    function confirmOrder() {
                        ...
                        if (yn) {
                            $.ajax({
                                url: "${ctxPath}/order/success/${requestScope.productOrder.productOrder_code}",
                                type: "PUT",
                                data: null,
                                dataType: "json",
                                success: function (data) {
                                    if (data.success) {
                                        location.href = "${ctxPath}/order/success/${requestScope.productOrder.productOrder_code}";
                                    } else {
                                        alert("订单确认异常，请稍后再试！");
