...
function create(obj) {
    ...
    $.ajax({
        url: session.getItem("contextUrl")+"/orderItem",
        type: "PUT",
        data: {
        ...
        success: function (data) {
            if (data.success) {
                location.href = session.getItem("contextUrl")+"/order/create/byCart?order_item_list=" + data.orderItemIDArray;
                return true;
            } else {
                alert("购物车商品结算异常，请稍候再试！");
                location.href = session.getItem("contextUrl")+"/cart";
            }
        },
        beforeSend: function () {
        },
